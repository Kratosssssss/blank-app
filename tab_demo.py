...
with tab2:
    ...
    st.text_area("2. Language translation",
                 ...
                 height=100)
    st.text_area("3. Name entity reconnection", value='''
    Step 1. Translate
    ...
    st.text_area("7. Intent detection",
                 value="Step 6. Conduct intention classification for each key points summarized by step 3.", height=100)
...
with tab3:
    ...
    for intent, value in intent_options.items():
        sentiment_values[intent] = st.slider(intent, -3, 3, value)

    # Topic Section
    st.subheader("3. Topic")
    topic_weight = st.number_input("Weight", 0, 100, 50, key="3")
    topic_options = {"Budget": 4, "Communication": 4, "Decision Making": 4, "Efficiency": 2, "Automation": 1, "AI": 5,
                     "Change": 3, "Others": 1}
    topic_values = {}
    for topic, value in topic_options.items():
        topic_values[intent] = st.slider(topic, -3, 3, value)

    # Save Button
    _, right_button_col = st.columns([14, 1])
    with right_button_col:
        st.button("Save", key="index=3")
        # st.success("Parameters saved successfully!", label_visibility="hidden")
